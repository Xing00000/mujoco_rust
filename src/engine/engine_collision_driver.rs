...
pub fn mj_max_contact(m: *const mjModel, g1: i32, g2: i32, mut has_margin: i32) -> i32 {
    ...
    const mjMAXCONPAIR: i32 = 4;
    const mjDSBL_MULTICCD: i32 = 1 << 19;
    const mjDSBL_NATIVECCD: i32 = 1 << 17;
    const mjENBL_OVERRIDE: i32 = 1 << 0;

    // SAFETY: m is a valid pointer, g1/g2 are valid geom indices (caller contract)
    unsafe {
        ...
        if type1 == mjGEOM_HFIELD || type2 == mjGEOM_HFIELD {
            let typ: i32 = if type1 == mjGEOM_HFIELD { type2 } else { type1 };
        ...
        if type1 == mjGEOM_PLANE || type2 == mjGEOM_PLANE {
            let typ: i32 = if type1 == mjGEOM_PLANE { type2 } else { type1 };
            return match typ {
                mjGEOM_CAPSULE => 2,
        ...
        if ((*m).opt.disableflags & mjDSBL_NATIVECCD) != 0 {
            return if is_multiccd != 0 { 5 } else { 1 }; // mesh-mesh or mesh-box with libccd
        }
