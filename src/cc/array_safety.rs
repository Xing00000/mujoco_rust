...
pub fn strcmp_arr(lhs: [char; 0], rhs: [char; 0]) -> i32 {
    // WARNING: signature changed — verify body
    // Previous params: (lhs : [char ; 0], rhs : [char ; 0])
    // Previous return: i32
    todo ! ()
}
...
pub fn sprintf_arr(dest: [char; 0], format: *const i8) -> i32 {
    // WARNING: signature changed — verify body
    // Previous params: (dest : [char ; 0], format : * const i8)
    // Previous return: i32
    todo ! ()
}
...
pub fn strcat_arr(dest: [char; 0], src: *const i8) -> *mut i8 {
    // WARNING: signature changed — verify body
    // Previous params: (dest : [char ; 0], src : * const i8)
